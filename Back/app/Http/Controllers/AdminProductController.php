    ...
    public function update(Request $request, $id)
    {
        ...
        try {
            ...
            $validated = $request->validate([
                ...
                'stock' => 'integer|min:0',
                'image.*' => 'nullable|image|mimes:jpeg,png,jpg,webp|max:2048',
                'deleted_image.*' => 'nullable|string|max:255',
                'category_id' => 'exists:categories,id',
            ]);

            $product->update($validated);

            // si de nouvelles ont étés ajoutés, on les ajoute à la bd
            if ($request->hasFile('image')) {
                foreach ($request->file('image') as $file) {
                    $path = $file->store('product_images', 'public');
                    Log::info('Image stored', ['path' => $path]);
            ...
            // si des images sont à supprimer
            if ($request->has('deleted_image')) {
                $deletedImages = $request->input('deleted_image'); // récupère un tableau

                    foreach ($deletedImages as $imageName) {
                        Storage::disk('public')->delete($imageName);

                        $product->image()->where('name', $imageName)->delete();

                        Log::info('Old image deleted', ['path' => $imageName]);
                    }
