    ...
    export default function ProductsManagementPage() {
        ...
        const handleAddEdit = async (e: React.FormEvent) => {
            ...
            try {
                ...
                if (formData.image) {
                    if (images.length === 5){
                        setErrors({image: "You cant add more images !"})
                    } else {
                ...
                //si des images ont étés supprimées
                if (formData.deleted_images){
                    formData.deleted_images.forEach((image) => {
                        data.append("deleted_image[]", image);
                    });
                }
                ...
                fetchProducts();
                setModalOpen(false);
                setFormData({ name: "", description: "", price: "", stock: "", image: [], deleted_images: [], category_id: "" });
                setEditProduct(null);
            } catch (error: any) {
        ...
        const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
            ...
            if (files && files.length > (5 - images.length)){
                alert("You can choose only 5 images maximum")
            ...
            const deleteImage = (idImage: number, nameImage: string) => {

                // On cache l'image dans le DOM
                const trElement = document.querySelector(`tr[data-id="${idImage}"]`);
                trElement?.classList.add("hidden");

                console.log("taille du tableau d'image avant : " + images.length)

                setImages(((images?? []).filter(image => image.name !== nameImage)));

                setDeletedImages(prev => [...prev, nameImage]);

                // Ajouter le nom de l'image supprimée dans deleted_images de formData
                setFormData(prevFormData => {
                    // On récupère l'ancien tableau deleted_images, ou tableau vide
                    const prevDeleted = prevFormData.deleted_images ?? [];
                    return {
                        ...prevFormData,
                        // On ajoute nameImage (ou éventuellement une version modifiée) à deleted_images
                        deleted_images: [...prevDeleted, nameImage],
                        // Met à jour aussi les images restantes dans formData.image (optionnel si tu veux)
                        image: (prevFormData.image ?? []).filter(image => image.name !== nameImage),
                    };
                });

                console.log("taille du tableau d'image : " + images.length)
                                    ...
                                    <form onSubmit={handleAddEdit} className="space-y-6">
                                        ...
                                        <div>
                                            ...
                                            {editProduct && images.length > 0 && (
                                                <div className="mt-4 border rounded-xl  bg-white p-8">
                                                    <table className="min-w-full text-sm text-gray-700 border border-gray-50">
                                                        <thead className="bg-gray-200 text-pink-800 uppercase text-xs">
                                                        <tr>
                                                        ...
                                                        <tbody>
                                                        {images.map((image) => (
                                                            <tr className="hover:bg-gray-50  border border-gray-50  ">
                                                                <td data-id={image.id} className="px-4 py-2  border border-gray-50">
                                                                    <img
                                                                    src={"http://localhost:8000/storage/" + image.name}
                                                                    alt={image.name}
                                                                    key={image.id}
                                                                    className="w-16 h-16 object-cover rounded-xl shadow-md ring-2 ring-white"
                                                                />
                                                                ...
                                                                <td className="px-4 py-2">
                                                                    <button type="button"   onClick={() => deleteImage(image.id, image.name)}
                                                                            className="bg-transparent border border-red-900 ">
                                                                        <span className="text-red-900 p-2">Delete</span>
                                                                    </button>
                                                                </td>
                                                            </tr>
                                                        ))}
                                            ...
                                            <div className="relative" hidden={images.length === 5}>
                                                <input
                                                    type="file"
                                                    name="image"
                                                    ref={fileInputRef}
